...
-- Fix leagues policies: drop restrictive, recreate as permissive
...
CREATE POLICY "Anyone authenticated can view public leagues" ON public.leagues FOR SELECT TO authenticated USING ((is_public = true) OR (id IN (SELECT get_user_league_ids(auth.uid()))));
...
CREATE POLICY "Members can view league members" ON public.league_members FOR SELECT TO authenticated USING (league_id IN (SELECT get_user_league_ids(auth.uid())));
...
CREATE POLICY "League members can view feed" ON public.activity_feed FOR SELECT TO authenticated USING (league_id IN (SELECT get_user_league_ids(auth.uid())));
CREATE POLICY "Users can post to feed" ON public.activity_feed FOR INSERT TO authenticated WITH CHECK ((user_id = auth.uid()) AND (league_id IN (SELECT get_user_league_ids(auth.uid()))));
...
CREATE POLICY "League members can view matchups" ON public.matchups FOR SELECT TO authenticated USING (league_id IN (SELECT get_user_league_ids(auth.uid())));
...
-- Also allow anon to read leagues for join flow
DROP POLICY IF EXISTS "Anon can read leagues" ON public.leagues;
CREATE POLICY "Anon can read leagues" ON public.leagues FOR SELECT TO anon USING (true);
